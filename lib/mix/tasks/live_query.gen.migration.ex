defmodule Mix.Tasks.LiveQuery.Gen.Migration do
  @moduledoc """
  Generates a migration that creates all PostgreSQL triggers and trigger functions for all tables that occur in any `use LiveQuer.Notify, tables: [...]` list.
  """
  use Mix.Task
  ...
  defp postgresql_trigger_function() do
    """
    CREATE OR REPLACE FUNCTION live_query__notify_about_changes_in_table() RETURNS trigger AS $live_query__notify_about_changes_in_table$
      BEGIN
        SELECT pg_notify(TG_TABLE_NAME, NULL);

        RETURN NULL;
      END;
    $live_query__notify_about_changes_in_table$ LANGUAGE plpgsql;
    """
  end
  ...
  defp postgres_trigger_for_table(table) do
    ...
    AFTER INSERT OR UPDATE OR DELETE ON #{table}
      FOR EACH STATEMENT EXECUTE FUNCTION live_query__notify_about_changes_in_table();
    """
  end
end
